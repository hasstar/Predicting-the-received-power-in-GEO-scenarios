"""
===================================================================================
High-Orbit Satellite Signal Loss Prediction - XGBoost Model with RFE
===================================================================================

[Task Description]
Predict satellite signal loss using atmospheric parameters with XGBoost algorithm
and Recursive Feature Elimination (RFE) to find optimal feature subset
...
2. Key Advantages of XGBoost
   - Speed: Faster than traditional gradient boosting
   - Performance: Often achieves best results
   - Flexibility: Many tunable hyperparameters
   - Regularization: L1 (Lasso) and L2 (Ridge) regularization
   - Tree Pruning: Uses max_depth and then prunes backward
   - Built-in Cross-Validation

3. Recursive Feature Elimination (RFE)
   - Start with all features and train model
   - Rank features by importance
   - Remove least important feature
   - Repeat until only 1 feature remains
   - Track performance at each step
   - Find optimal feature subset

4. Evaluation Metrics
   - R^2: Coefficient of Determination (0 to 1, higher is better)
...
class XGBoostRFETrainer:
    """
    XGBoost Recursive Feature Elimination Trainer

    This class implements complete training pipeline including:
    ...
    def __init__(self, test_size=0.4, random_state=42):
        ...
        print("=" * 80)
        print("XGBoost Recursive Feature Elimination Trainer Initialized")
    ...
    def train_xgboost(self, X_train, y_train, X_test, y_test, feature_names):
        ...
        # Train model (no early stopping for compatibility)
    ...
    def recursive_feature_elimination(self):
        ...
        [RFE Process]
        For each iteration:
        1. Train XGBoost model with current features
        2. Evaluate on train and test sets
        3. Get feature importance
        4. Record current feature list
        5. Remove least important feature
        6. Repeat until 1 feature remains
        """
        ...
        print("  3. Get feature importance ranking")
        ...
        log_file = open('XGBoost_RFE_detailed_log.txt', 'w', encoding='utf-8')
        log_file.write("=" * 100 + "\n")
        log_file.write("XGBoost RFE Detailed Log - Feature List for Each Iteration\n")
        log_file.write("=" * 100 + "\n\n")
        ...
        print(f"  Detailed feature log saved: XGBoost_RFE_detailed_log.txt")
    ...
    def plot_rfe_curves(self, save_path='XGBoost_RFE_performance_curves.png'):
        """
    ...
    def plot_feature_importance(self, save_path='XGBoost_feature_importance_top20.png'):
        """
    ...
    def plot_prediction_scatter(self, save_path='XGBoost_prediction_scatter.png'):
        """
    ...
    def plot_residual_analysis(self, save_path='XGBoost_residual_analysis.png'):
        """
    ...
    def save_results_summary(self, save_path='XGBoost_training_results_summary.txt'):
        """
        ...
        with open(save_path, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("XGBoost Training Results Summary\n")
            f.write("=" * 80 + "\n\n")
    ...
    def save_results_to_excel(self, save_path='XGBoost_training_results_detailed.xlsx'):
        """
        Save to Excel
        """
    ...
    def run_complete_pipeline(self):
        ...
        print("\n")
        print("=" * 80)
        print(" " * 20 + "XGBoost RFE Training Pipeline Started")
        print("=" * 80)
        ...
        print("=" * 80)
        print("\nGenerated files:")
        print("  1. XGBoost_RFE_performance_curves.png")
        print("  2. XGBoost_feature_importance_top20.png")
        print("  3. XGBoost_prediction_scatter.png")
        print("  4. XGBoost_residual_analysis.png")
        print("  5. XGBoost_training_results_summary.txt")
        print("  6. XGBoost_training_results_detailed.xlsx")
        print("  7. XGBoost_RFE_detailed_log.txt - DETAILED FEATURE LIST PER ITERATION")


def generate_standalone_script(trainer, output_filename="run_prediction.py"):
    """
    【生成器】自动生成一个包含模型和特征的独立Python脚本
    """
    import pickle
    import base64
    import os

    print(f"\n[正在生成独立预测脚本: {output_filename} ...]")

    # 1. 获取最佳模型和特征列表
    best_model = trainer.best_result['model']
    best_features = trainer.best_result['features']

    # 2. 将模型对象转换成一串 base64 字符串
    # 这样就不需要额外的 .json 文件了，模型直接变成了代码里的一行字符串
    ...
    REQUIRED_FEATURES = {best_features}
    ...
    def get_user_input():
        ...
        print("  高轨卫星信号衰减预测工具")
        print("="*60)
        print(f"注意：需要输入 {{len(REQUIRED_FEATURES)}} 个特征的值。")
        print("直接回车将使用默认值(0.0)。\\n")

        data = {{}}
        for feature in REQUIRED_FEATURES:
            try:
                val_str = input(f"请输入 [{{feature}}]: ")
    ...
    if __name__ == "__main__":
        ...
        while True:
            ...
            try:
                ...
                print("\\n" + "-"*40)
                print(f"预测结果 (Loss): {{prediction:.4f}} dB")
                print("-"*40 + "\\n")
    ...
    print("你可以把这个文件发给任何人，直接双击运行即可，无需其他依赖文件。")
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 80)
    print("Thank you! Check XGBoost_RFE_detailed_log.txt for complete feature lists.")
    print("=" * 80)

    # 【新增】生成单文件预测工具
    generate_standalone_script(trainer)
