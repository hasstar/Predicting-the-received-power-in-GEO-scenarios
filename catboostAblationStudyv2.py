"""
===================================================================================
High-Orbit Satellite Signal Loss Prediction - Final Ablation Study
===================================================================================

[Goal]
1. Compare Surface-Only Model vs. Stratified (Proposed) Model.
2. Track RMSE_Overall (Total), RMSE_Rainy, and RMSE_Cloudy at every iteration.
3. Calculate percentage improvement for Total RMSE to validate overall gain.

[Output Files]
1. Ablation_Detail_Surface.csv    : Full log for Surface model
2. Ablation_Detail_Stratified.csv : Full log for Stratified model
3. Ablation_Summary_Best.csv      : Comparison table with % improvement
...
class CatBoostIterativeAblation:
    ...
    def run_rfe_process(self, initial_features, scenario_name):
        """
        Runs RFE and records FULL history including Overall RMSE.
        """
        ...
        for step in range(total_steps):
            ...
            # --- METRICS CALCULATION ---
            # Total/Overall RMSE (The main comparison metric)
            rmse_overall = np.sqrt(mean_squared_error(self.y_test, y_pred))

            # Scenario Metrics
            rmse_rain = np.sqrt(
                mean_squared_error(self.y_test[rain_mask], y_pred[rain_mask])) if rain_mask.sum() > 0 else np.nan
            rmse_cloud = np.sqrt(
                mean_squared_error(self.y_test[cloud_mask], y_pred[cloud_mask])) if cloud_mask.sum() > 0 else np.nan
            ...
            history.append({
                'n_features': n_feats,
                'RMSE_Overall': rmse_overall,  # This is your Total RMSE
            ...
            if step % 5 == 0 or n_feats == 1:
                print(
                    f"      [Iter {step + 1}/{total_steps}] Feats={n_feats:2d} | Overall={rmse_overall:.4f} | Cloud={rmse_cloud:.4f}")
    ...
    def run_full_study(self):
        ...
        # Select best iteration based on min Overall RMSE
        ...
        print(
            f"{'Rainy RMSE':<20} | {best_surface['RMSE_Rainy']:<15.4f} | {best_stratified['RMSE_Rainy']:<15.4f} | {'-':<15}")
        print(
        ...
        data = [
            {
                'Metric': 'Overall RMSE',
                'Surface_Value': best_surface['RMSE_Overall'],
                'Stratified_Value': best_stratified['RMSE_Overall'],
                'Improvement_Pct': imp_overall
            },
            {
                'Metric': 'Cloudy RMSE',
                'Surface_Value': best_surface['RMSE_Cloudy'],
                'Stratified_Value': best_stratified['RMSE_Cloudy'],
                'Improvement_Pct': imp_cloud
            }
        ]
        pd.DataFrame(data).to_csv('Ablation_Summary_Best.csv', index=False)
        print("  > Summary table saved to 'Ablation_Summary_Best.csv'")
    ...
    def plot_curves(self, hist_surf, hist_strat):
        ...
        rmse_surf_overall = [h['RMSE_Overall'] for h in hist_surf]  # Total RMSE
        rmse_surf_cloud = [h['RMSE_Cloudy'] for h in hist_surf]

        n_strat = [h['n_features'] for h in hist_strat]
        rmse_strat_overall = [h['RMSE_Overall'] for h in hist_strat]  # Total RMSE
        rmse_strat_cloud = [h['RMSE_Cloudy'] for h in hist_strat]

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

        # Plot 1: Overall RMSE (Total RMSE) - This is what you requested to compare
        ax1.plot(n_surf, rmse_surf_overall, 'o--', color='gray', label='Surface Only', alpha=0.7)
        ax1.plot(n_strat, rmse_strat_overall, 's-', color='#d62728', label='Stratified (Proposed)')
        ax1.invert_xaxis()
        ax1.set_xlabel('Number of Features')
        ax1.set_ylabel('Overall RMSE (dB)')
        ax1.set_title('Overall Performance (Total RMSE)')
        ax1.legend()
        ax1.grid(True, alpha=0.3)

        # Plot 2: Cloudy RMSE
        ax2.plot(n_surf, rmse_surf_cloud, 'o--', color='gray', label='Surface Only', alpha=0.7)
        ax2.plot(n_strat, rmse_strat_cloud, 's-', color='#d62728', label='Stratified (Proposed)')
        ax2.invert_xaxis()
        ax2.set_xlabel('Number of Features')
        ax2.set_ylabel('Cloudy (No-Rain) RMSE (dB)')
        ax2.set_title('Cloudy Scenario Performance')
        ax2.legend()
        ax2.grid(True, alpha=0.3)

        plt.tight_layout()
        plt.savefig('Ablation_Overall_and_Cloudy.png', dpi=300)
        print("  > Plot saved to 'Ablation_Overall_and_Cloudy.png'")
